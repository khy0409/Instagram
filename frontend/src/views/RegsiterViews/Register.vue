...
// const tests = () => {
//   modalMessage.value = '휴대폰으로 인증번호가 발송되었습니다.'
//   modalStatus(true)
// }
...
const checkEmailOrPhone = async () => {
  ...
  if (isPhoneNumber) {
    ...
    // if (emailOrPhone.value.length == 11) {
    //   setError(emailOrPhoneError, '엔터를 눌러 인증번호 받기를 진행해주세요', emailOrPhoneChecked)
    //   return
    // }
  } else {
    // 이메일 형식 체크
...
const register = async () => {
  ...
  try {
    ...
    if (emailOrPhone.value.includes('@')) {
      ...
      // 전화번호로 가입 처리
      const isVerified = await setupRecaptcha(emailOrPhone.value)
      if (!isVerified) {
        throw new Error('전화번호 인증에 실패했습니다.')
      }
      modalMessage.value = 'SMS로 전송된 인증 코드를 입력해주세요.'
      modalStatus(true)
      // SMS 코드 입력 처리는 별도의 함수나 컴포넌트에서 처리해야 합니다.
      return
    }
    ...
    if (emailOrPhone.value.includes('@')) {
      ...
      modalMessage.value =
        '이메일 인증 링크가 발송되었습니다. 이메일을 확인하여 인증을 완료해주세요.'
      modalStatus(true)
      setTimeout(() => {
